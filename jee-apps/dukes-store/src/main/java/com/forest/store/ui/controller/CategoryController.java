...
import com.forest.store.wiring.catalog.CategoryBean;
...
public class CategoryController implements Serializable {
	...
	@EJB
	private com.forest.store.wiring.catalog.CategoryBean ejbFacade;
	...
	public Category getSelected() {
		if (current == null) {
			current = ejbFacade.newCategoryInstance();
			selectedItemIndex = -1;
		}
		return current;
	}
	...
	public PageNavigation create() {
		try {
			getFacade().createCategory(current);
			JsfUtil.addSuccessMessage(ResourceBundle.getBundle(BUNDLE)
					.getString("CategoryCreated"));
			recreateModel();
	...
	public PageNavigation update() {
		try {
			getFacade().updateCategory(current);
			JsfUtil.addSuccessMessage(ResourceBundle.getBundle(BUNDLE)
	...
	private void performDestroy() {
		try {
			getFacade().removeCategory(current);
			JsfUtil.addSuccessMessage(ResourceBundle.getBundle(BUNDLE)
					.getString("CategoryDeleted"));
		} catch (Exception e) {
			JsfUtil.addErrorMessage(e, ResourceBundle.getBundle(BUNDLE)
					.getString("PersistenceErrorOccured"));
		}
	...
	public static class CategoryControllerConverter implements Converter {
		...
		public Object getAsObject(FacesContext facesContext,
			...
			CategoryController controller = (CategoryController) facesContext
					.getApplication()
					.getELResolver()
					.getValue(facesContext.getELContext(), null,
							"categoryController");

			return controller.ejbFacade.getCategory(getKey(value));
		}
