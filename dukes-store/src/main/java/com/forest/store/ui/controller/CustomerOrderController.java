...
public class CustomerOrderController implements Serializable {
	...
	private static final Logger logger = Logger
			.getLogger(CustomerOrderController.class.getCanonicalName());
	...
	public CustomerOrder getSelected() {
		if (current == null) {
			current = ejbFacade.newCustomerOrderInstance();
			selectedItemIndex = -1;
		}
		return current;
	}
	...
	public List<CustomerOrder> getMyOrders() {
		...
		if (user != null) {
			...
			if (myOrders.isEmpty()) {
				...
				logger.log(Level.FINEST, "Order amount:{0}", myOrders.get(0)
						.getAmount());
				return myOrders;
			}
	...
	private void performDestroy() {
		try {
			getFacade().removeCustomerOrder(current);

			JsfUtil.addSuccessMessage(ResourceBundle.getBundle(BUNDLE)
					.getString("CustomerOrderDeleted"));
		} catch (Exception e) {
	...
	public static class CustomerOrderControllerConverter implements Converter {
		...
		public Object getAsObject(FacesContext facesContext,
			...
			CustomerOrderController controller = (CustomerOrderController) facesContext
					.getApplication()
					.getELResolver()
					.getValue(facesContext.getELContext(), null,
							"customerOrderController");
			return controller.ejbFacade.getCustomerOrder(getKey(value));
		}
